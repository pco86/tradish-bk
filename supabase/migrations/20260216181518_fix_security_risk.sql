...
BEGIN
    ...
    SELECT vault.decrypted_secret INTO webhook_url FROM vault.decrypted_secrets WHERE name = 'add_occurrences_new_tradition';
    SELECT vault.decrypted_secret INTO api_key FROM vault.decrypted_secrets WHERE name = 'webhook_secret';
...
BEGIN
    -- Retrieve the secret values from the vault.decrypted_secrets view
    SELECT vault.decrypted_secret INTO webhook_url FROM vault.decrypted_secrets WHERE name = 'schedule_occurrence_generation';
    SELECT vault.decrypted_secret INTO api_key FROM vault.decrypted_secrets WHERE name = 'webhook_secret';
