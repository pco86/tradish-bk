...
CREATE POLICY "Date Rules are viewable for traditions that can be seen" ON tradition_date_rules FOR
SELECT
    TO authenticated,
    anon USING (
        EXISTS (
...
CREATE POLICY "Users can create date rules on traditions they own" ON tradition_date_rules FOR INSERT TO authenticated
WITH
    CHECK (
        EXISTS (
            SELECT
...
CREATE POLICY "Creators can update their completion date rules" ON tradition_date_rules FOR
...
CREATE POLICY "Users can delete their date rules" ON tradition_date_rules FOR DELETE TO authenticated USING (
    EXISTS (
        SELECT
            1
        FROM
